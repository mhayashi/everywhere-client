...
function createTip(message) {
  ...
  divprofile
    ...
    .append('<div class="everywhere-qtip-reply"><a href="javascript:void(0)" onclick="var ev = document.createEvent(\'Events\'); ev.initEvent(\'reply\', true, true); var target = document.getElementById(\'qtipManager\'); target.innerHTML=\''+message.username+'\'; target.dispatchEvent(ev);">Reply</a></div>');
...
function sendMessage() {
  ...
  var url = esc(document.location.href);  chrome.extension.sendRequest({ msgType: "update",
                                 url: url,
                                 message: message
                               });
  textarea.val("");
